...
class User {
  ...
  static async updateAccountInformation(id, { phone_number }) {
    const normalizedPhoneNumber = normalizePhoneNumber(phone_number);
    ...
    const query = `
      UPDATE users
      SET phone_number = ?, phone_number_hash = ?
  ...
  static async updateProfileByFirebaseUid(firebaseUid, {
    username,
    display_name,
    phone_number,
    discoverable_by_phone
  ...
  static async findByIdWithStats(userId) {
    ...
    user.follower_count = parseInt(rows[0].follower_count, 10);
    user.following_count = parseInt(rows[0].following_count, 10);
  ...
  static async findByUsername(username) {
    const query = `
      SELECT id, username, display_name, bio, created_at, avatar_url
      FROM users
  ...
  static async getUserFeed(
    ...
    const daysBack = Number.isFinite(Number(days)) && Number(days) > 0 ? Math.floor(Number(days)) : 1;

    if (!Number.isFinite(userId)) {
      throw new Error('currentUserId must be a number');
    }

    const intervalDays = Math.max(1, daysBack);
    ...
    let sql = `
      ...
      WHERE
        ...
        AND p.is_deleted = 0
        AND u.is_active = 1
        AND p.created_at >= (NOW() - INTERVAL ${intervalDays} DAY)
  ...
  async getFeed(limit = 20, offset = 0, daysBack = 2, includeOwn = false) {
    return User.getUserFeed(this.id, daysBack, limit, offset, includeOwn);
  }
